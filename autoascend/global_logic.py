    ...
    def current_strategy(self):
        ...
        while 1:
            ...
            if self.milestone == Milestone.BE_ON_FIRST_LEVEL:
                # hypothesis: Rangers and Tourists that leave the level-1 farm at XL7 avoid its
                # long, food-intensive final grind while retaining more safety than the rejected XL6 exit.
                condition = lambda: self.agent.blstats.experience_level >= (
                    7 if self.agent.character.role in (Character.RANGER, Character.TOURIST) else 8)
